...
// 前處理 //
void xml_read_name(string Path_name,std::vector<string> *image_name);
void xml_read_pts(string Path_name,std::vector<string> image_name,std::vector<std::vector<Point2f>> *model_LM_all);
void xml_read_name_MPIE(string Path_name,std::vector<string> *image_name);
void xml_read_pts_MPIE(string Path_name,std::vector<string> image_name,std::vector<std::vector<Point2f>> *model_LM_all);
void FRGC_img_show(string FRGC_model_image_path,std::vector<string> FRGC_image_name,std::vector<std::vector<Point2f>> model_LM_all);
void MPIE_img_show(string MPIE_file_image_path,std::vector<string> MPIE_image_name,std::vector<std::vector<Point2f>> MPIE_LM_all);

// 模型選擇 //
void model_choose_LMset(std::vector<std::vector<Point2f>> LM_in, std::vector<std::vector<Point2f>>* LM_out);
void draw_point(std::vector<std::vector<Point2f>> LM_in);
void model_choose(std::vector<std::vector<Point2f>> model_LM_all,std::vector<Point2f> MPIE_LM_each,std::vector<int>* choose_num, int* min_num);
void angle_calculate(std::vector<Point2f> LM_in, std::vector<float> *angle_out);
void draw_point_afterModelChoose(std::vector<Point2f> FRGC_1,std::vector<Point2f> FRGC_2,std::vector<Point2f> MPIE);

// 模型變形 //
void model_transform(std::vector<Point2f> model_LM, std::vector<Point2f> img_LM, Mat input_img, string FRGC_model_image_path, string FRGC_image_name, Mat ViewMatrix, Mat ProjMatrix, Mat *model_x_out, Mat *model_y_out, Mat *model_z_out, Mat *mask_out, Mat *img_out,std::vector<Point2f> *Reg_LM_out);
void model_transform_LMset(std::vector<Point2f> LM_in, std::vector<Point2f> *LM_out, Mat ViewMatrix, Mat ProjMatrix);
void volume_spline_interpolation(std::vector<Point2f> src, std::vector<Point2f> dst, Mat *coefficient, float *k_num);
float radial_basis_Fun(std::vector<Point2f> LM, int row_s, int col_s, float k);
float radial_basis_Fun_check(std::vector<Point2f> src, int row_s, int col_s, float k);
void model_transform_cood(Mat mask, Mat model_x, Mat model_y, Mat *model_x_out, Mat *model_y_out, std::vector<Point2f> src, Mat coefficient, float k_num);
float trans_cood(Mat model_x,Mat model_y, std::vector<Point2f> src, Mat coefficient, int row_s, int col_s, int dims, float k_num);
float radial_basis_Fun2(Mat model_x, Mat model_y,std::vector<Point2f> LM, int row_s, int col_s, int nums, float k_num);
void Create_face_mask(std::vector<Point2f> input_point, Mat &out_mask);
void Create_Leye_mask(std::vector<Point2f> input_point, Mat &out_mask);
void Create_Reye_mask(std::vector<Point2f> input_point, Mat &out_mask);
...
void write_model_LM(string savePath, std::vector<Point2f> Reg_LM, Mat model_x, Mat model_y, Mat model_z);
void write_model_old(string savePath, Mat msak, Mat img, Mat model_x, Mat model_y, Mat model_z);
void write_model_LM_old(string savePath, std::vector<Point2f> Reg_LM, Mat model_x, Mat model_y, Mat model_z);
...
int main(int argc, char* argv[])
{
	...
	dlib::frontal_face_detector detector_dlib = dlib::get_frontal_face_detector(); //內建人臉偵測model, 目測 +-45
	...
	// load model //
	string FRGC_model_file_name="FRGC-model-AM";
	string FRGC_image_name="04203d439";
	...
	dlib::cv_image<dlib::bgr_pixel> cimg(FRGC_image);
	int number_size=0;
	cv::Point2f points_plot; 
	std::vector<dlib::full_object_detection> shapes; // 存點位置
	// Detect faces 
	std::vector<dlib::rectangle> faces = detector_dlib(cimg);
	std::vector<cv::Point2f> model_LM_each;
	if (faces.size()!=0)
	{
		...
		//char bufCount[100];
		std::vector<cv::Point2f> dlib_LM;
		for (int i=0;i<number_size;i++)
		{
			points_plot.x=shapes[0].part(i).x();
			points_plot.y=shapes[0].part(i).y();
			dlib_LM.push_back(points_plot);
			//circle(FRGC_image_D,points_plot,2,cv::Scalar(0,255,255),(-1));
			//sprintf(bufCount,"%d",(i+1));
			//putText( FRGC_image_D, bufCount, dlib_LM[dlib_LM.size()-1], 0, 0.25, cv::Scalar(255,0,0), 1, CV_AA);
			//imshow("FRGC_image_final",FRGC_image_D);waitKey(0);
		}
		//imshow("FRGC_image_final",FRGC_image_D);waitKey(1);
		dlib_LM_set(dlib_LM, &model_LM_each);
	}
	for (std::vector<Point2f>::iterator LM_pt=model_LM_each.begin();LM_pt!=model_LM_each.end();++LM_pt)
	{
	...
	while(1)
	{
		...
		std::vector<Point2f> test_LM_each;
		dlib::cv_image<dlib::bgr_pixel> cimg(test_img);
		int number_size=0;
		cv::Point2f points_plot; 
		std::vector<dlib::full_object_detection> shapes; // 存點位置
		// Detect faces 
		...
		if (faces.size()!=0)
		{
			...
			//imshow("FRGC_image_final",FRGC_image_D);waitKey(1);
			dlib_LM_set(dlib_LM, &test_LM_each);
		}
		for (std::vector<Point2f>::iterator LM_pt=test_LM_each.begin();LM_pt!=test_LM_each.end();++LM_pt)
		{
			circle(test_img_D,*LM_pt,1,CV_RGB(0,255,0),-1);
		}
		imshow("test_img_D",test_img_D);inputKey=waitKey(5);
		...
		if (p_flag)
		{
			...
			string pic_no;
			stringstream float2string2; //string to float
			float2string2 << pic;
			float2string2 >> pic_no;
			string save_model_title=data_file_title+"model_rec_test/Demo/"+pic_no+"/";
			...
			string save_depth_m = save_model_title +pic_no+"_lambda_"+lambda_str+"_m.png";
			imwrite(save_depth_m,mask_d);
			string save_depth_i = save_model_title +pic_no+"_lambda_"+lambda_str+"_i.png";
			imwrite(save_depth_i,test_img_D);
			string save_depth_f = save_model_title +pic_no+"_lambda_"+lambda_str+"_f.png";
...
void xml_read_name(string Path_name,std::vector<string> *image_name)
{
	std::vector<string> temp_image_name; 
	FileStorage FS_NT_R;
...
void xml_read_pts(string Path_name,std::vector<string> image_name,std::vector<std::vector<Point2f>> *model_LM_all)
{
	std::vector<std::vector<Point2f>> temp_model_LM_all;
	FileStorage FS_LDT_R;
...
void FRGC_img_show(string FRGC_model_image_path,std::vector<string> FRGC_image_name,std::vector<std::vector<Point2f>> model_LM_all)
{
	for (int i=0; i<FRGC_image_name.size();i++)
	{
		...
		int point_number=1;
		for (std::vector<Point2f>::iterator LM_pt=model_LM_all[i].begin();LM_pt!=model_LM_all[i].end();++LM_pt)
		{
			...
			cv::putText(model_ori_img_d,point_number_str,cv::Point2f(temp.x-5,temp.y-5),FONT_HERSHEY_PLAIN , 1, CV_RGB(0,0,255), 1);
			int2string.str("");
			int2string.clear();
			point_number=point_number+1;
		}
		cv::imshow("model_ori_img_d",model_ori_img_d);inputKey=cv::waitKey(0);
	}
}
void MPIE_img_show(string MPIE_file_image_path,std::vector<string> MPIE_image_name,std::vector<std::vector<Point2f>> MPIE_LM_all)
{
	...
	for (int i=0; i<MPIE_image_name.size();i++)
	{
		...
		int point_number=1;
		for (std::vector<Point2f>::iterator LM_pt=MPIE_LM_all[i].begin();LM_pt!=MPIE_LM_all[i].end();++LM_pt)
		{
			...
			int2string<<point_number;
			int2string>>point_number_str;
			cv::putText(model_ori_img_d_mpie,point_number_str,cv::Point2f(temp.x-5,temp.y-5),FONT_HERSHEY_PLAIN , 1, CV_RGB(0,0,255), 1);
			int2string.str("");
			int2string.clear();
			point_number=point_number+1;
		}
		cv::imshow("model_ori_img_d",model_ori_img_d_mpie);inputKey_mpie=cv::waitKey(0);
	}
}
...
void model_choose_LMset(std::vector<std::vector<Point2f>> LM_in, std::vector<std::vector<Point2f>>* LM_out)
{
	std::vector<std::vector<Point2f>> model_LM_all=LM_in;
	std::vector<std::vector<Point2f>> model_LM_all_after=LM_in;
...
void draw_point(std::vector<std::vector<Point2f>> LM_in)
{
	...
	for (int i=0;i<LM_in.size();i++)
	{
		...
		cv::imshow("point",img);cv::waitKey(0);
		int2string.str("");
		int2string.clear();
		point_number=point_number+1;
	}
...
void model_choose(std::vector<std::vector<Point2f>> model_LM_all,std::vector<Point2f> MPIE_LM_each, std::vector<int>* choose_num, int* min_num)
{
	...
	std::vector<float> s_dis_compare=s_dis;
	...
	*choose_num=num;
	...
	std::vector<float> angLe_2d;
	angle_calculate(MPIE_LM_each, &angLe_2d);
	

	std::vector<float> angle_error;
	...
	std::vector<float> angle_error_compare=angle_error;
...
void angle_calculate(std::vector<Point2f> LM_in, std::vector<float> *angle_out)
{
	...
	std::vector<int> number_Leyebow(point_Leyebow, point_Leyebow + sizeof(point_Leyebow)/sizeof(point_Leyebow[0]));
	...
	std::vector<int> number_Reyebow(point_Reyebow, point_Reyebow + sizeof(point_Reyebow)/sizeof(point_Reyebow[0]));
	v.clear();
	...
	std::vector<int> number_Leye(point_Leye, point_Leye + sizeof(point_Leye)/sizeof(point_Leye[0]));
	v.clear();
	...
	std::vector<int> number_Reye(point_Reye, point_Reye + sizeof(point_Reye)/sizeof(point_Reye[0]));
	v.clear();
	...
	std::vector<int> number_Nose(point_Nose, point_Nose + sizeof(point_Nose)/sizeof(point_Nose[0]));
	v.clear();
	...
	std::vector<int> number_Mouth(point_Mouth, point_Mouth + sizeof(point_Mouth)/sizeof(point_Mouth[0]));
	v.clear();
	...
	std::vector<int> number_F_contour(point_F_contour, point_F_contour + sizeof(point_F_contour)/sizeof(point_F_contour[0]));
	v.clear();
...
void model_transform(std::vector<Point2f> model_LM, std::vector<Point2f> img_LM, Mat input_img, string FRGC_model_image_path, string FRGC_image_name, Mat ViewMatrix, Mat ProjMatrix, Mat *model_x_out, Mat *model_y_out, Mat *model_z_out, Mat *mask_out, Mat *img_out, std::vector<Point2f> *Reg_LM_out)
{
	...
	Mat Reg_img_temp=input_img.clone(); Reg_img_temp.setTo(0);
	Point2f dst_point;
	...
	std::vector<Point2f> Reg_LM_each_move;
	...
	//Mat merge_2=0.5*FRGC_image+0.5*Reg_img_temp_crop;
	//// LM點位確認 //
	...
	//}
	//imshow("merge_2",merge_2);waitKey(0);

	//讀取 Model 的 x y z r g b nx ny nz//
	string ply_model_path=FRGC_model_image_path+FRGC_image_name+"/"+FRGC_image_name+"-no-Color-t.ply";
	string file_open_FRGC_index=FRGC_model_image_path+FRGC_image_name+"/"+FRGC_image_name+"-index.txt";
	string file_open_FRGC_model_mask=FRGC_model_image_path+FRGC_image_name+"/"+FRGC_image_name+"-mask.ppm";
	...
	// reconstruction //
	std::vector<Point2f> model_LM_after_2;
	std::vector<Point2f> img_LM_after_2;
	Set_the_LM(model_LM,&model_LM_after_2);
	Set_the_LM(Reg_LM_each_move,&img_LM_after_2);
	std::vector<cv::Point2f> src_pt=img_LM_after_2;
	std::vector<cv::Point2f> dst_pt=model_LM_after_2;
	int aff_num=0;
	...
	string warp_txt_path=title+"Warp-txt/persp_v5.txt";
	set_persp_aff(src_pt,&aff_src_pt,warp_txt_path,&aff_num);
	set_persp_aff(dst_pt,&aff_dst_pt,warp_txt_path,&aff_num);
	Mat_<float> colorWarp_temp(480,640);colorWarp_temp.setTo(0);
	cv::Mat_<uchar> affine_Z_mask_t(480,640);
	affine_Z_mask_t.setTo(0);
	cv::Mat_<uchar> temp_mask(480,640);
	temp_mask.setTo((0));
	std::vector<Mat> crop_img_s;
	std::vector<Mat> crop_img_s_after;
	cv::split(Reg_img_temp_crop,crop_img_s);
	cv::split(Reg_img_temp_crop,crop_img_s_after);
	crop_img_s_after[0].setTo(0);
	crop_img_s_after[1].setTo(0);
	crop_img_s_after[2].setTo(0);
	for(int i=0; i<aff_num; i++)
	{
		aff_warp(aff_src_pt[i], aff_dst_pt[i], crop_img_s[0], &colorWarp_temp,&temp_mask);
		colorWarp_temp.copyTo(crop_img_s_after[0],temp_mask);
	...
	Mat crop_img_after;
	merge(crop_img_s_after,crop_img_after);
	...
	for (int i=0;i<affine_Z_mask_t.rows;i++)
		for (int j=0;j<affine_Z_mask_t.cols;j++)
			...
			crop_img_after.copyTo(crop_img_after,affine_Z_mask_t);
			// Recovering Light //
			Mat albode_ref;
			...
			std::vector<cv::Point2f> src_pt_2=model_LM;
			std::vector<cv::Point2f> dst_pt_2=Reg_LM_each_move;

			//將ref_pt的點依分好的點位做區塊,存至aff_src_pt//
			int aff_num_2=0;
			std::vector<std::vector<cv::Point2f>> aff_src_pt_2;
			std::vector<std::vector<cv::Point2f>> aff_dst_pt_2;
...
void volume_spline_interpolation(std::vector<Point2f> src, std::vector<Point2f> dst, Mat *coefficient, float *k_num)
{
	...
	std::vector<float> err_count;
	std::vector<float> k_count;
	std::vector<Mat> coeff_count;
	...
	std::vector<float> err_count_compare=err_count;
...
void Create_face_mask(std::vector<Point2f> input_point, Mat &out_mask)
{
	...
	std::vector<int> number(Mask_point_glass_mid, Mask_point_glass_mid + sizeof(Mask_point_glass_mid)/sizeof(Mask_point_glass_mid[0]));
...
void calculate_Light(Mat image_i,Mat albedo_ref,Mat normal_x,Mat normal_y,Mat normal_z, Mat affine_Z_mask, Mat* L)
{
	...
	if (albedo_ref.channels()==3)
	{
		...
		std::vector<Mat> albedo_ref_HSV_s;
...
{	
	...
	std::vector<float> s_dis_compare=error_compare;
...
void aff_warp(vector<cv::Point2f> src_pnt, vector<cv::Point2f> dst_pnt,cv::Mat img_in,cv::Mat *img_out,cv::Mat *mask_out)
{
	...
	cv::drawContours(TMask, contours, -1, cv::Scalar(155), -1, CV_AA, hierarchy, 0); //利用 mask 畫輪廓
