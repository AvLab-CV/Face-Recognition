...
{	
	...
	// load train image //
	string test_image_title="C:\\Users\\Max\\Pictures\\comps\\";
	...
	for (int i = 0; i < test_img_name.size(); i++)
	{
		...
		vector<string> Name_Tar_test;
		 ...
		 string test_sPath=recog_data_sPath_title+test_img_name[i]+"/";
		...
		for (int j = 0; j < test_img_name_in.size(); j++)
		{
			...
			//{
				...
				string img_load_path=test_img_path_in+test_img_name_in[j];
				...
				//cvShowImage("Mat_TT_out",Mat_TT_out);waitKey(1);
				...
				string TT_save_name=test_sPath+test_img_name_in[j];
				cout<<TT_save_name<<endl;
				imwrite(TT_save_name,Mat_TT_out_show*255);
				//dct
				...
				Name_Tar_test.push_back(test_img_name_in[j].substr(0,3));
